...
C
      real*8 fval(ivecd,8)                ! output returned
C
...
C  derivatives:
C
         if((ict(2)+ict(5)+ict(6)+ict(8)).gt.0) then
            axp=6.0*xp*xpi
            axbarp=-axp
            bxp=xp*(3.0*xp-2.0)
...
C
         if((ict(3)+ict(5)+ict(7)+ict(8)).gt.0) then
            ayp=6.0*yp*ypi
            aybarp=-ayp
            byp=yp*(3.0*yp-2.0)
...
C
         if((ict(4)+ict(6)+ict(7)+ict(8)).gt.0) then
            azp=6.0*zp*zpi
            azbarp=-azp
            bzp=zp*(3.0*zp-2.0)
